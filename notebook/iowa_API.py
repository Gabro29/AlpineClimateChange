...
def get_data_from_query(query, station, pathtosave):
    """Our main method for downloading data for a given station based on the query.

    Args:
      query (string): Query string with all parameters already included.
      station (string): Station ID.
      pathtosave (string): Path where the output file will be saved.
    """
    # Construct the full URI by concatenating the service URL and the query
    uri = SERVICE + query
    print(f"Downloading: {station}")
    data = download_data(uri)
    outfn = os.path.join(pathtosave, f"{station}.txt")
    with open(outfn, "w", encoding="ascii") as fh:
        fh.write(data)
